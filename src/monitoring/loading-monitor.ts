...
export class ModelLoadingMonitor {
  ...
  startMonitoring(modelId: string, baseURL: string): void {
    ...
    console.info(`[@jc3212/opencode-models-discovery] Started monitoring model loading`, { modelId, baseURL })
  ...
  private async checkLoadingProgress(modelId: string, baseURL: string, getModelsFn?: () => Promise<string[]>): Promise<void> {
    try {
      ...
      if (isLoaded) {
        ...
        this.stopMonitoring(modelId)
        console.info(`[@jc3212/opencode-models-discovery] Model loading completed`, {
          modelId,
  ...
  private updateState(
    ...
    if (currentState.status !== status) {
      if (status === 'loaded') {
        console.info(`[@jc3212/opencode-models-discovery] Model loading completed`, { modelId })
      } else if (status === 'error') {
        console.warn(`[@jc3212/opencode-models-discovery] Model loading failed`, { modelId, error })
      } else if (status === 'loading') {
        console.info(`[@jc3212/opencode-models-discovery] Model loading started`, { modelId })
      }
    }
  }
